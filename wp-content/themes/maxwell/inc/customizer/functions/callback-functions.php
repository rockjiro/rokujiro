 ...
 * @return bool
 */
function maxwell_control_post_content_callback( $control ) {

	// Check if excerpt mode is selected.
	if ( 'excerpt' === $control->manager->get_setting( 'maxwell_theme_options[post_content]' )->value() ) :
		return true;
	else :
		return false;
	endif;
 ...
 * @return bool
 */
function maxwell_slider_activated_callback( $control ) {

	// Check if Slider is turned on.
	if ( true === $control->manager->get_setting( 'maxwell_theme_options[slider_blog]' )->value() ) :
		return true;
	elseif ( true === $control->manager->get_setting( 'maxwell_theme_options[slider_magazine]' )->value() ) :
		return true;
	else :
		return false;
	endif;
